...
class Trainer(object):
    def __init__(self, data_config):
        ...
        self.image_feats = np.load('./data/{}/image_feat.npy'.format(args.dataset))
        self.text_feats = np.load('./data/{}/text_feat.npy'.format(args.dataset))
        self.image_feat_dim = self.image_feats.shape[-1]
        self.text_feat_dim = self.text_feats.shape[-1]

        self.ui_graph = self.ui_graph_raw = pickle.load(open('./data/' + args.dataset + '/train_mat','rb'))
    ...
    def train(self):
        ...
        for epoch in range(args.epoch):
            ...
            for idx in tqdm(range(n_batch)):
                ...
                write_path = "./t_SNE_G/distribution/dir_draw"
